...
function Dashboard() {
  ...
  if (userData?.vendor_id !== 0) {
    useEffect(() => {
      if (!userData?.vendor_id || userData?.vendor_id === "undefined") {
        navigate("/login", {
    ...
    useEffect(() => {
      const fetchChartData = async () => {
        try {
          const order_response = await axiosAPIInstance.get(
            `vendor/vendor-orders-report-chart/${userData?.vendor_id}/`
          );
          setOrderChartData(order_response.data);
  
          const product_response = await axiosAPIInstance.get(
            `vendor/vendor-products-report-chart/${userData?.vendor_id}/`
          );
